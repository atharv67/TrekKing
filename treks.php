...
$find_treks="select distinct name,state,difficulty,height,no_of_days,description,price,pic1 from tbl_treks ";
if (isset($_GET["month"])||isset($_GET["difficulty"])||isset($_GET["state"])) {
  $find_treks.="where ";
if (isset($_GET["month"])) {
  $month=$_GET["month"];
  $find_treks.="MONTH(start_date)=$month ";
  if (isset($_GET["difficulty"])||isset($_GET["state"])){
    $find_treks.="and ";
  }
}
if (isset($_GET["difficulty"])) {
  $difficulty=$_GET["difficulty"];
  $find_treks.="difficulty='$difficulty' ";
  if (isset($_GET["state"])){
    $find_treks.="and ";
  }
}
if (isset($_GET["state"])) {
  $state=$_GET["state"];
  $find_treks.="state='$state'";
}
}
}
// $find_query="";
// if(isset($month_check)){
//   $find_query.=$month_find;
//   if($difficulty_check||$state_check){
//     $find_query.="INTERSECT ";
//   }
// }
// if(isset($difficulty_check)){
//   $find_query.=$difficulty_find;
//   if($state_check){
//   $find_query.="INTERSECT ";
// }
// }
// if(isset($state_check)){
//   $find_query.=$state_find;
...
<form action="search_page.php">
  <input type="submit" name="filter" value="Change Filters" class="btn btn-outline-dark" style="margin-left:700px; ">
</form>
